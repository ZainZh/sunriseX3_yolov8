...
def postprocess(model_output,
    ...
    for index, mbbox in enumerate(mbbox_buffer):
        mbbox_buffer[index] = mbbox * model_output[1].properties.scale_data

    lbbox_buffer = model_output[2].buffer.astype(float)

    for index, lbbox in enumerate(lbbox_buffer):
        lbbox_buffer[index] = lbbox * model_output[2].properties.scale_data

    pred_sbbox = sbbox_buffer.reshape([1, 84, 84, 3, 85]).transpose([0, 3, 1, 2, 4])

    pred_mbbox = mbbox_buffer.reshape([1, 42, 42, 3, 85]).transpose([0, 3, 1, 2, 4])

    pred_lbbox = lbbox_buffer.reshape([1, 21, 21, 3, 85]).transpose([0, 3, 1, 2, 4])

    # pred_sbbox = model_output[0].buffer.reshape([1, 84, 84, 3, 85]).transpose([0, 3, 1, 2, 4])
    # pred_mbbox = model_output[1].buffer.reshape([1, 42, 42, 3, 85]).transpose([0, 3, 1, 2, 4])
    # pred_lbbox = model_output[2].buffer.reshape([1, 21, 21, 3, 85]).transpose([0, 3, 1, 2, 4])
...
def get_classes(class_file_name='../../src/yolov5/coco_classes.names'):
